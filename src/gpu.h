...
public:
    gpu_metrics metrics;
    std::string drm_node;                   // DRM node (renderD128 etc)
    ...
    void pause() {
        if (nvidia) nvidia->pause();
        if (amdgpu) amdgpu->pause();
        if (fdinfo) fdinfo->pause();
    }

    void resume() {
        if (nvidia) nvidia->resume();
        if (amdgpu) amdgpu->resume();
        if (fdinfo) fdinfo->resume();
    }

    bool is_apu() {
        return amdgpu ? amdgpu->is_apu : false;
    }

    std::shared_ptr<Throttling> throttling() {
        if (nvidia) return nvidia->throttling;
        if (amdgpu) return amdgpu->throttling;
        return nullptr;
    }
...
public:
    ...
    std::shared_ptr<GPU> active_gpu() {
        if (available_gpus.empty()) return nullptr;

        for (auto gpu : available_gpus) {
            if (gpu->is_active) return gpu;
        }

        // if no GPU marked as active, return last
    ...
    std::vector<std::shared_ptr<GPU>> selected_gpus() {
        ...
        if (!params()->gpu_list.empty()) {
            for (unsigned index : params()->gpu_list) {
                if (index < available_gpus.size() && available_gpus[index])
...
private:
    ...
    const std::array<std::string, 9> supported_drivers = {
        "amdgpu", "nvidia", "i915", "xe", "panfrost",
        "panthor", "msm_dpu", "msm_drm", "kgsl_adreno"
    };
};
