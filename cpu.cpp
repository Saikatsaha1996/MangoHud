...
bool CPUStats::UpdateCPUData()
{
    ...
    if (!file.is_open()) {
        SPDLOG_WARN("Failed to open " PROCSTATFILE ", using cpuidle fallback");

        size_t numCores = std::thread::hardware_concurrency();
        if (numCores == 0) numCores = 1; // fallback if unknown

        float totalLoad = 0.0f;
        int countedCores = 0;

        for (size_t cpu = 0; cpu < numCores; cpu++) {
            std::string idlePath = "/sys/devices/system/cpu/cpu" + std::to_string(cpu) + "/cpuidle/state0/time";

            std::ifstream idleFile(idlePath);
            if (!idleFile.is_open())
                continue;
            ...
            // total time is approx since boot
            unsigned long long total_now = idle_now;

            // sum over other cpuidle states
            for (int s = 1; s < 10; s++) {
                std::string path = "/sys/devices/system/cpu/cpu" + std::to_string(cpu) + "/cpuidle/state" + std::to_string(s) + "/time";
                std::ifstream f(path);
            ...
            // store previous if first call
            if (m_prevCpuIdle.size() <= cpu) {
                m_prevCpuIdle.resize(cpu + 1, idle_now);
                m_prevCpuTotal.resize(cpu + 1, total_now);
                continue; // cannot compute delta yet
            }

            unsigned long long deltaIdle = idle_now - m_prevCpuIdle[cpu];
            unsigned long long deltaTotal = total_now - m_prevCpuTotal[cpu];

            float load = 0.0f;
            if (deltaTotal > 0)
                load = 1.0f - ((float)deltaIdle / (float)deltaTotal);

            totalLoad += load * 100.0f;
            countedCores++;

            // store for next iteration
            m_prevCpuIdle[cpu] = idle_now;
            m_prevCpuTotal[cpu] = total_now;
        }

        if (countedCores > 0) {
            m_cpuDataTotal.percent = std::clamp(totalLoad / (float)countedCores, 0.0f, 100.0f);
            m_updatedCPUs = true;
            return true;
        }
    ...
    do {
        if (!std::getline(file, line)) {
            ...
            ret = true;
            calculateCPUData(m_cpuDataTotal, usertime, nicetime, systemtime, idletime,
            ...
            &ioWait, &irq, &softIrq, &steal, &guest, &guestnice) == 11) {
            ...
            if (cpu_count + 1 > m_cpuData.size() || m_cpuData[cpu_count].cpu_id != cpuid) {
                SPDLOG_DEBUG("Cpu id '{}' mismatch, reiniting", cpuid);
                return Reinit();
            }

            CPUData& cpuData = m_cpuData[cpu_count];
            calculateCPUData(cpuData, usertime, nicetime, systemtime, idletime,
                             ioWait, irq, softIrq, steal, guest, guestnice);
            cpuid = -1;
            cpu_count++;
